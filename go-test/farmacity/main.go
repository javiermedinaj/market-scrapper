...
func saveHistoricalData(output Output) error {
	...
	dailyFile := filepath.Join(dailyDir, "farmacity-ofertas.json")
	...
	currentFile := filepath.Join("..", "data", "farmacity-ofertas.json")
...
func main() {
	baseURL := "https://www.farmacity.com/api/catalog_system/pub/products/search"
	pageSize := 50
	maxProducts := 2500
	allProducts := []Product{}
	...
	for from := 0; from < maxProducts; from += pageSize {
		...
		var pageProducts []FarmacityProduct
		...
		for _, fp := range pageProducts {
			if len(fp.Items) > 0 && len(fp.Items[0].Sellers) > 0 {
				...
				link := "https://www.farmacity.com/" + fp.LinkText + "/p"
	...
	fmt.Printf("✅ Scraping de Farmacity completado. Total productos: %d\n", len(allProducts))
}
