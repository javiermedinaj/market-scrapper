...
export default function Analytics() {
  ...
  const convertToChartData = (categories: Record<string, { averagePrice: number }>) => {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      const [categoriesRes, storesRes, availableCatsRes] = await Promise.all([
        ...
        fetch("/api/analytics/stores-comparison"),
        fetch("/api/analytics/available-categories"),
      ]);
      ...
      if (availableCatsData.categories?.length > 0) {
  ...
  const fetchCheapestByCategory = useCallback(async (category: string) => {
    try {
      const res = await fetch(
        `/api/analytics/cheapest-by-category/${encodeURIComponent(category)}`
      );
  ...
  const supermarketStores = convertToChartData(storesComparison.supermarkets?.stores || {});
  const pharmacyStores = convertToChartData(storesComparison.pharmacies?.stores || {});
              ...
              {categoryStats.length > 0 ? (
                categoryStats.map((cat, i) => (
                  <tr key={i} className="border-b border-zinc-800 hover:bg-zinc-800">
                    ...
                    <td className="py-3 px-2 md:px-4 text-green-400 font-semibold text-xs md:text-sm">
                      {cat.cheapestStore}
                    </td>
