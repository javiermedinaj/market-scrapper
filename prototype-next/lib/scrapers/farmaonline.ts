...
export async function scrapeFarmaonline(): Promise<StoreData> {
  const storeName = "FarmaOnline";
  const storeSlug = "farma";
  const baseUrl = "https://www.farmaonline.com";
  const pageSize = 50;
  ...
  for (let from = 0; from < maxProducts; from += pageSize) {
    ...
    console.log(`📄 FarmaOnline: Obteniendo productos ${from}-${to}...`);
    const response = await fetchWithRetry(url);

    const vtexProducts = (await response.json()) as VtexProduct[];
    console.log(`✅ FarmaOnline: Página procesada. Total: ${products.length}`);
...
export default scrapeFarmaonline;
